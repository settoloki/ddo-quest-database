...
class UpdateQuestRequest extends FormRequest
{
    ...
    {
        // For now, allow all users to update quests
    ...
    {
        $questId = $this->route('quest');

        return [
            'name' => 'sometimes|required|string|max:255|unique:quests,name,' . $questId,
            'slug' => 'nullable|string|max:255|unique:quests,slug,' . $questId,
